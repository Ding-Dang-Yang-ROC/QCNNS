...
def Train_Pyt(one,img):
  ...
  for i in range (n_batch):
    ...
    pred1=torch.matmul(Con_Unitary(Par1[1]),pred0).reshape(4,4,2,2,batch).permute(0,2,1,3,4).reshape(2,2,2,2,2,2,batch).permute(0,2,3,5,1,4,6).reshape(16,4*batch)
    pred2=torch.matmul(Con_Unitary(Par1[2]),pred1).reshape(2,2,2,2,2,2,batch).permute(6,0,4,1,2,5,3).reshape(batch,64)
    pred3=pred2.pow(2)
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range (n_batch):
    ...
    pred1=torch.matmul(cm1,pred0).reshape(4,4,2,2,batch).permute(0,2,1,3,4).reshape(2,2,2,2,2,2,batch).permute(0,2,3,5,1,4,6).reshape(16,4*batch)
    pred2=torch.matmul(cm2,pred1).reshape(2,2,2,2,2,2,batch).permute(6,0,4,1,2,5,3).reshape(batch,64)
    pred3=pred2.pow(2)
