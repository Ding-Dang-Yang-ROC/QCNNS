...
def Read_MNIST(data):
    ...
    fr.close()
    img2 = torch.tensor(img, dtype=Dtype).reshape(-1, N_fn * N_fn).to(dev2)
...
def One_Hot(target):
  ...
  t2=torch.tensor(t).reshape(len(target),10).to(dev2)
  return t2
...
def OutPara(par1,par2):
  ...
  par=par1.reshape(-1).tolist()+par2.reshape(-1).tolist()
