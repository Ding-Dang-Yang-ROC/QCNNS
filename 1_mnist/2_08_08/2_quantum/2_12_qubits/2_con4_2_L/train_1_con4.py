...
def Train_Pyt(one,img):
  ...
  for i in range (n_batch):
    ...
    pred1=torch.matmul(Con_Unitary(Par1[1]),pred0).reshape(4,4,4,4,2,2,2,2,batch).permute(8,0,4,1,5,2,6,3,7).reshape(batch,64*64)
    pred2=pred1.pow(2)
...
def Pred_Pyt(img,par1,par2):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      ...
      batch=end_frm-beg_frm
    img1=img[num[beg_frm:end_frm]].reshape(batch,2,4,2,4,2,4,2,4).permute(2,4,6,8,1,3,5,7,0).reshape(256,16*batch)
    pred0=torch.matmul(cm0,img1).reshape(4,4,4,4,2,2,2,2,batch).permute(4,0,5,1,6,2,7,3,8).reshape(4,2,4,2,4,2,4,2,batch) \
      .permute(0,2,4,6,1,3,5,7,8).reshape(256,16*batch)
    pred1=torch.matmul(cm1,pred0).reshape(4,4,4,4,2,2,2,2,batch).permute(8,0,4,1,5,2,6,3,7).reshape(batch,64*64)
    pred2=pred1.pow(2)
