...
def Para():
  par=torch.rand((64*64,10),device=dev,dtype=Dtype,requires_grad=True)
  return par
...
def Train_Pyt(one,img):
  ...
  for i in range (n_batch):
    ...
    pred0=img[num[beg_frm:end_frm]].reshape(batch,64*64).pow(2)
...
def Pred_Pyt(img,par):
  ...
  for i in range (n_batch):
    ...
    if end_frm>sam:
      end_frm=sam
      batch=end_frm-beg_frm
    pred0=img[num[beg_frm:end_frm]].reshape(batch,64*64)
