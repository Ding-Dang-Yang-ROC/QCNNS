...
def Read_Par():
  with open('../../../../2_quantum/2_12_qubits/1_con4_1_L/test00/Result/par',"r") as f:
    data=[float(num) for line in f for num in line.strip().split()]
  par1=torch.tensor(data[:S_Ker*S_Ker],dtype=torch.float32)
  par2=torch.tensor(data[S_Ker*S_Ker:],dtype=torch.float32).reshape(64*64,10)
  return par1,par2
