...
def quantum_circuit(input_state,cm0,cm1):
  ...
  qml.DepolarizingChannel(noise_level, wires=4)
  qml.DepolarizingChannel(noise_level, wires=5)
  phase_damping = 0.03
  qml.PhaseDamping(phase_damping, wires=0)
  qml.PhaseDamping(phase_damping, wires=1)
  qml.PhaseDamping(phase_damping, wires=2)
  qml.PhaseDamping(phase_damping, wires=3)
  qml.PhaseDamping(phase_damping, wires=4)
  qml.PhaseDamping(phase_damping, wires=5)
...
def Read_Par():
  with open('../../../../2_quantum/1_6_qubits/2_con4_2_L/test00/Result/par',"r") as f:
  ...
  par2=torch.tensor(data[2*S_Ker*S_Ker:],dtype=torch.float32).reshape(64,10)
  return par1,par2
